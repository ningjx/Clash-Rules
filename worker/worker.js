    ...
    try {
      ...
      try {
        ...
        if (bwResp.ok) {
          ...
          const now = new Date();
          let expireDate = new Date(now.getFullYear(), now.getMonth(), bwInfo.bw_reset_day_of_month);
          
          // 如果当前日期已经过了重置日，则设置为下个月的重置日
          if (now.getDate() > bwInfo.bw_reset_day_of_month) {
            expireDate = new Date(now.getFullYear(), now.getMonth() + 1, bwInfo.bw_reset_day_of_month);
          }
