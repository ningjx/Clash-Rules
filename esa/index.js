...
module.exports = async function main(request, context) {
  ...
  try {
    ...
    console.log(`[${requestId}] User-Agent: ${request.headers['user-agent'] || 'unknown'}`);
    ...
    const [bwResp, upstreamResp, templateResp] = await Promise.all([
      fetch(bwUrl).catch(e => { 
        console.error(`[${requestId}] BW fetch error:`, e); 
        return null; 
      }),
      fetch(queryUrl).catch(e => { 
        console.error(`[${requestId}] Upstream fetch error:`, e); 
        return null; 
      }),
      fetch(templateUrl).catch(e => { 
        console.error(`[${requestId}] Template fetch error:`, e); 
        return null; 
      })
    ]);
    ...
    if (bwResp && bwResp.status >= 200 && bwResp.status < 300) {
      try {
    ...
    if (templateResp && templateResp.status >= 200 && templateResp.status < 300) {
      try {
